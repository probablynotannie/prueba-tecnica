...
export default function Pokemons() {
  ...
  const allPokemons = pokemonDetailsQueries.map((q) => q.data).filter(Boolean);
          ...
          <section>
            {visiblePokemons.length === 0 ? (
              <div className="col-span-5 flex flex-col items-center text-red-400 bg-red-50 p-6 rounded">
                ...
                <p>
                  De <span className="font-semibold">{data.count}</span>{" "}
                  pokemons, has tenido que elegir uno que no existe.. Asi no
                  ganas. :(
                </p>
              </div>
            ) : (
